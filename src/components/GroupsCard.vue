<template>
  <div class="bg-base-100 rounded-xl">
    <h2 class="text-center flex justify-center items-center font-bold text-xl m-6">Grupos y canales disponibles</h2>
    <div class="form-control flex-row items-center">
      ...
      <button v-else @click="getAllChats(true)" class="mr-5 btn btn-circle btn-ghost"><IconReload /></button>
    </div>
    <div class="divider mx-auto w-11/12 m-2"></div>
    <i v-if="chatsLoading" class="text-center flex justify-center items-center text-base mx-4 mb-4">Cargando grupos...</i>
    <i v-else-if="filteredGroups.length === 0" class="text-center flex justify-center items-center text-base mx-4 mb-4">No se ha encontrado ningún grupo</i>
    <div v-else class="overflow-x-auto">
      <table class="table">
        ...
        <tbody>
          <tr v-for="group in paginatedGroups" :key="paginatedGroups.indexOf(group)" class="hover" @click="group.selected = !group.selected">
            <td>
              <label>
            ...
            <td>
              <div class="tooltip" :data-tip="isSupported && copied ? '¡Copiado!' : 'ID: ' + group.id" @click.stop="() => { if(isSupported) copy(group.id.toString()) }">
...
async function getAllChats(forceReplace = false) {
  if(!clientStore.client){
    ...
    if(forceReplace || !chatsStore.date || chatsStore.chats.length === 0 || (chatsStore.date && chatsStore.date < Date.now() - 86400000)) {
